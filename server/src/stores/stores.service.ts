...
export class StoresService implements OnModuleInit, OnModuleDestroy {
  ...
  async create(data: Prisma.StoreCreateInput, userId: number) {
    ...
    if (
      ...
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedContactEmail)
        ...
        throw new BadRequestException('Wrong contact email format');
      }
  ...
  throw new BadRequestException('No valid image paths provided');}
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      for (const item of data.pavilions ?? []) {
        ...
        if (
          ...
          normalizedNumber.includes('сумм') ||
  ...
  private validateBillingDetails(
    ...
    if (!companyName || !legalAddress || !inn) {
      throw new BadRequestException(
        'If any billing detail is provided, all of them (companyName, legalAddress, inn) must be provided',
      );
    }
