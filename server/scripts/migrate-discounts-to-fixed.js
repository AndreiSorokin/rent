const { PavilionStatus, PrismaClient } = require('@prisma/client');
const { addMonths, endOfMonth, startOfMonth } = require('date-fns');
...
function getMonthlyDiscountTotal(discounts, period) {
...
async function refreshMonthlyLedger(pavilionId, period) {
...
async function main() {
  ...
  const earliestAffectedPeriodByPavilion = new Map();
  ...
  for (const [pavilionId, firstPeriod] of earliestAffectedPeriodByPavilion.entries()) {
    ...
    console.log(
      `Recalculated ledgers for pavilion #${pavilionId} starting from ${firstPeriod
        .toISOString()
        .slice(0, 10)}.`,
    );
  }
